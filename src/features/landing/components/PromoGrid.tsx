...
export function PromoGrid() {
  ...
  return (
    <section className="bg-[#f6f7fb] py-16 md:py-20" id="build-a-bundle">
      <div className="mx-auto grid max-w-[1700px] gap-7 px-4 lg:grid-cols-[1fr_1fr]">
        <article
          className="relative min-h-[420px] overflow-hidden rounded-[14px] bg-black p-7 text-white md:min-h-[580px] md:p-9"
          style={{
        ...
        >
          ...
          <img
            alt="5-hour ENERGY bundle packs"
            className="absolute bottom-6 left-1/2 w-[88%] max-w-[760px] -translate-x-1/2 object-contain md:bottom-8"
            src={bundleBoxes}
          />
        </article>
        ...
        <div className="grid gap-7">
          ...
          <article className="relative min-h-[280px] overflow-hidden rounded-[14px] bg-black p-7 text-white md:min-h-[276px] md:p-9">
            ...
            <div className="absolute bottom-0 left-4 right-4 flex h-[150px] items-end justify-center gap-1 md:h-[168px] md:gap-2">
              {productLine.map((image, index) => (
                <img alt="" className="h-full w-auto object-contain drop-shadow-[0_12px_18px_rgba(0,0,0,.4)]" key={`${image}-${index}`} src={image} />
              ))}
            </div>
