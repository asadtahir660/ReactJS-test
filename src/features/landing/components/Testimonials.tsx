  ...
  return (
    <section className="overflow-hidden bg-white py-24 md:py-28">
      <div className="mx-auto max-w-[1660px] px-4">
        <SectionHeading
          description="Lorem ipsum dolor sit amet, consectetur ading elit. Lorem ipsum dolor sit amet, conetur ading elit."
          highlight="Testimonials"
          title="View Testimonials From Our Customers"
        />
        ...
        <div className="relative mt-16">
          <div className="grid items-center gap-6 lg:grid-cols-[0.72fr_1.28fr_0.72fr]">
            {visibleCards.map((testimonial) => (
              <article
                className={`testimonial-card rounded-[10px] bg-white px-7 py-10 text-center shadow-[0_12px_38px_rgba(0,0,0,0.09)] md:px-12 ${
                  testimonial.position === 'active' ? 'testimonial-card-active' : 'testimonial-card-side'
                }`}
            ...
            <div className="flex items-center gap-2">
              {testimonials.map((testimonial, index) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all ${index === activeIndex ? 'w-12 bg-[#ffe500]' : 'w-3 bg-[#dcdcdc] hover:bg-[#ffe500]'}`}
