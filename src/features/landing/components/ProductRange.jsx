...
export function ProductRange() {
    return (<section className="relative overflow-hidden bg-white px-4 pb-20 text-white" id="shop">
      ...
      <div className="relative mx-auto max-w-[1760px]">
        ...
        <div className="mt-16 grid gap-7 sm:grid-cols-2 xl:grid-cols-4">
          {productCards.map((product) => (<article className="product-card group mx-auto min-w-0 overflow-hidden rounded-[22px] bg-white text-black shadow-[0_20px_45px_rgba(0,0,0,0.16)] transition duration-300 hover:-translate-y-2 hover:shadow-[0_26px_58px_rgba(0,0,0,0.22)]" key={product.name}>
              <div className="rounded-[22px] m-[24px] border-white bg-black">
                <div className={`relative grid aspect-square place-items-center overflow-hidden rounded-[10px] bg-[radial-gradient(var(--tw-gradient-stops))] ${product.glow} to-black`}>
                  <img alt={product.name} className="h-[88%] w-auto object-contain drop-shadow-[0_26px_44px_rgba(0,0,0,0.6)] transition duration-500 group-hover:scale-105" src={product.image}/>
                  <button aria-label={`Add ${product.name} to bag`} className="absolute bottom-3 right-3 grid h-11 w-11 place-items-center rounded-[8px] bg-white text-black shadow-[0_10px_18px_rgba(0,0,0,0.25)] transition hover:bg-black hover:text-white" type="button">
