...
export function FeaturedProductSection() {
    ...
    function move(direction) {
        setActiveIndex((current) => (current + direction + featuredProducts.length) % featuredProducts.length);
    }
    return (<section className="overflow-hidden bg-white py-24 md:py-32" id="featured-products">
      <div className="mx-auto max-w-[1740px] px-4">
        ...
        <div className="mt-[72px] grid items-center gap-12 lg:grid-cols-[1fr_minmax(420px,600px)_1fr]">
          ...
          <div className="grid gap-9">
            {product.facts.map((fact) => (<div className="grid grid-cols-[78px_1fr] items-center gap-5 md:grid-cols-[96px_1fr]" key={fact.title}>
                {/*
            ...
            */}
                <div className="flex h-[72px] w-[72px] items-center justify-center md:h-20 md:w-20">
                  <FactIcon className="h-[72px] w-[72px] object-contain md:h-20 md:w-20" src={fact.icon}/>
                </div>
          ...
          <div className="relative mx-auto grid min-h-[660px] w-full max-w-[630px] place-items-end md:min-h-[710px]">
            ...
            <img alt={product.title} className="featured-product-image relative z-10 mx-auto max-h-[720px] w-auto max-w-[84%] object-contain" src={product.image}/>
          </div>
          ...
          <div className="max-w-xl lg:pl-8">
            ...
            <div className="mt-16 flex gap-4 md:mt-20">
              <button aria-label="Previous product" className="grid h-16 w-16 place-items-center rounded-full border border-[#ddd] text-3xl text-[#777] transition hover:border-black hover:bg-black hover:text-white" onClick={() => move(-1)} type="button">
                &larr;
              </button>
              <button aria-label="Next product" className="grid h-16 w-16 place-items-center rounded-full bg-[#ffe500] text-3xl text-black transition hover:bg-black hover:text-white" onClick={() => move(1)} type="button">
                &rarr;
              </button>
            </div>
          </div>
