...
export function EnergyNavbar() {
    ...
    return (<header className="absolute left-0 right-0 top-0 z-40 px-3 py-4 sm:px-5">
      <nav className="mx-auto w-full max-w-[1780px] overflow-hidden rounded-[8px] bg-white shadow-[0_12px_40px_rgba(0,0,0,0.25)]">
        <div className="flex min-h-[64px] items-center justify-between gap-4 px-4 py-3 sm:px-6">
          <button aria-expanded={menuOpen} aria-label="Toggle menu" className="grid h-11 w-11 shrink-0 place-items-center rounded-[8px] border border-black/10 text-black transition hover:bg-black hover:text-white md:hidden" onClick={() => setMenuOpen((current) => !current)} type="button">
          ...
          <div className="hidden items-center gap-8 text-sm font-semibold text-[#222] xl:flex">
            {navItems.map((item) => (<a className="transition hover:text-[#d8cc00]" href={navTarget(item)} key={item}>
                {item}
              </a>))}
          </div>
        ...
        {menuOpen ? (<div className="grid gap-1 border-t border-black/10 px-4 pb-4 pt-2 text-sm font-bold text-[#222] md:hidden">
            {navItems.map((item) => (<a className="rounded-[8px] px-3 py-3 transition hover:bg-black hover:text-white" href={navTarget(item)} key={item} onClick={() => setMenuOpen(false)}>
                {item}
              </a>))}
