      ...
      <main className="app-shell py-8">
        ...
        <Card className="mt-6 p-4">
          <div className="grid gap-3 md:grid-cols-[1fr_190px]">
            <Input label="Search by name or email" onChange={(event) => setSearch(event.target.value)} placeholder="Search users" value={search}/>
        ...
        {!loading && !error ? (<>
            <div className="mt-6 hidden overflow-hidden rounded-lg border border-line bg-white md:block">
              <table className="w-full border-collapse text-left">
                ...
                <tbody>
                  {visibleUsers.map((user) => (<tr className="border-t border-line" key={user.id}>
            ...
            <div className="mt-6 grid gap-4 md:hidden">
              {visibleUsers.map((user) => (<Card className="p-5" key={user.id}>
            ...
            {visibleUsers.length === 0 ? (<Card className="mt-6 p-8 text-center">
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button disabled={page === totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))} variant="secondary">
                  Next
                </Button>
              </div>
            </div>
