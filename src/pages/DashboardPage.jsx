...
export function DashboardPage() {
    ...
    return (<div className="min-h-screen bg-surface">
          ...
          <aside className="grid content-start gap-4">
            ...
            <Card className="p-5">
              ...
              <form className="mt-4 grid gap-4" onSubmit={handleCreateTask}>
                <Input error={errors.title} label="Title" name="title" onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} placeholder="Follow up with new prospect" value={form.title}/>
                <TextArea error={errors.description} label="Description" name="description" onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} placeholder="Add the next action or important context" value={form.description}/>
          ...
          <section className="grid content-start gap-4">
            <Card className="p-4">
              <div className="grid gap-3 md:grid-cols-[1fr_190px]">
                <Input label="Search by title" onChange={(event) => setSearch(event.target.value)} placeholder="Search tasks" value={search}/>
                <label className="block">
                  <span className="mb-2 block text-sm font-semibold text-ink">Sort</span>
                  <select className="min-h-11 w-full rounded-md border border-line bg-white px-3.5 py-2.5 text-sm text-ink outline-none focus:border-route-600 focus:ring-2 focus:ring-route-100" onChange={(event) => setSortMode(event.target.value)} value={sortMode}>
            ...
            {filteredTasks.length === 0 ? (<Card className="p-8 text-center">
                ...
                {filteredTasks.map((task) => (<Card className="p-5" key={task.id}>
                    <div className="grid gap-4 md:grid-cols-[1fr_auto]">
                      ...
                      <div className="grid gap-2 sm:grid-cols-3 md:min-w-72 md:grid-cols-1">
                        <Button onClick={() => handleStatusChange(task.id, task.status === 'Pending' ? 'Completed' : 'Pending')} variant="secondary">
      ...
      <Modal open={Boolean(editingTask)} onClose={() => setEditingTask(null)} title="Edit Task">
        {editingTask ? (<form className="grid gap-4" onSubmit={handleEditSubmit}>
            <Input error={errors.title} label="Title" onChange={(event) => setEditingTask((current) => (current ? { ...current, title: event.target.value } : current))} value={editingTask.title}/>
            <TextArea error={errors.description} label="Description" onChange={(event) => setEditingTask((current) => (current ? { ...current, description: event.target.value } : current))} value={editingTask.description}/>
            <label className="block">
              <span className="mb-2 block text-sm font-semibold text-ink">Status</span>
              <select className="min-h-11 w-full rounded-md border border-line bg-white px-3.5 py-2.5 text-sm text-ink outline-none focus:border-route-600 focus:ring-2 focus:ring-route-100" onChange={(event) => setEditingTask((current) => (current ? { ...current, status: event.target.value } : current))} value={editingTask.status}>
