...
const navLinkClass = ({ isActive }) => `rounded-md px-3 py-2 text-sm font-semibold transition ${isActive ? 'bg-route-50 text-route-700' : 'text-muted hover:bg-white hover:text-ink'}`;
export function Navbar() {
    ...
    return (<header className="sticky top-0 z-30 border-b border-line bg-surface/95 backdrop-blur">
      <nav className="app-shell flex min-h-[74px] items-center justify-between gap-4 py-3">
        ...
        <button className="rounded-md border border-line bg-white px-3 py-2 text-sm font-semibold text-ink md:hidden" onClick={() => setOpen((value) => !value)} type="button">
          Menu
        </button>
      ...
      {open ? (<div className="border-t border-line bg-surface md:hidden">
